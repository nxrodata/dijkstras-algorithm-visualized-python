...
class Box:
    def __init__(self, x, y, box_width, box_height):
        ...
        self.visited = False 
        self.neighbors = []
        self.prior = None # The box from which this box was first discovered
            ...
            if begin_search:
                ...
                if len(self.queue) > 0 and searching:
                    ...
                    if current_box == target_box:
                        temp = current_box
                        while temp.prior != self.start_box: # While the current box is not the start box
                            self.path.append(temp.prior) # Append the prior box to the path
                            temp = temp.prior # Set the current box to the prior box
                            temp.draw(self.window, PATH_COLOR) # Color the path box
